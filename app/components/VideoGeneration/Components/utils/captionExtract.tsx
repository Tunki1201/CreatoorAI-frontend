export default async function captionExtract(message: string): Promise<string> {
    try {
      const messages = [
        { role: "user", content: message }, // Prepare the message format expected by the API
      ];
  
      const response = await fetch("/api/captionEx", {
        method: "POST",
        headers: {
      ...
      console.log("Caption API Extract response:", data);
